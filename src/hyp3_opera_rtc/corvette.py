...
from rtc.core import check_ancillary_inputs, save_as_cog
...
from rtc.rtc_s1_single_job import (
...
def process_child_runconfig(
    path_runconfig_burst, path_burst_logfile=None, flag_full_logfile_format=None, keep_burst_runconfig=False
):
    """
...
def run_parallel(cfg: RunConfig, logfile_path, flag_logger_full_format):
    ...
    burst_runconfig_list, burst_log_list = split_runconfig(
        cfg, output_path_child, burst_product_id_list, child_scratch_path, logfile_path
    )
    ...
    with multiprocessing.Pool(num_workers) as p:
        processing_result_list = p.starmap(
            process_child_runconfig, zip(burst_runconfig_list, burst_log_list, repeat(flag_logger_full_format))
        )
    t_end_parallel = time.time()
    logger.info('Child processes has completed. ' f'Elapsed time: {t_end_parallel - t_start_parallel} seconds.')
    # ------  End of parallelized burst processing ------

    # Check if there are any failed child processes
    all_child_processes_successful = processing_result_list.count(0) == len(burst_runconfig_list)
    ...
    if all_child_processes_successful:
        ...
        msg_failed_child_proc = (
            'Some of the child process(es) from ' 'burst runconfig (listed below) ' 'did not complete succesfully:\n'
        )
        ...
        for index_child, processing_result in enumerate(processing_result_list):
            ...
            if os.path.isdir(output_dir_bursts) and len(os.listdir(output_dir_bursts)) == 0:
            ...
            msg_failed_child_proc += f'"{burst_runconfig_list[index_child]}" for burst ID ' f'"{burst_id}"\n'
    ...
    for burst_index, (burst_id, burst_pol_dict) in enumerate(cfg.bursts.items()):
        ...
        burst_hdf5_in_output = os.path.join(output_path_child, burst_id, f'{burst_product_id}.{hdf5_file_extension}')
        if not save_bursts:
        ...
        if save_rtc_anf:
            if save_secondary_layers_as_hdf5:
                rtc_anf_file = (
                    f'NETCDF:"{burst_hdf5_in_output}":' f'{DATA_BASE_GROUP}/' f'{layer_hdf5_dict[layer_name_rtc_anf]}'
                )
            else:
                rtc_anf_file = (
                    f'{output_dir_sec_bursts}/{burst_product_id}' f'_{layer_name_rtc_anf}.{imagery_extension}'
                )
        ...
        if product_type != STATIC_LAYERS_PRODUCT_TYPE:
            ...
            for pol in pol_list:
                if save_imagery_as_hdf5:
                    geo_burst_pol_filename = f'NETCDF:{burst_hdf5_in_output}:' f'{DATA_BASE_GROUP}/' f'{pol}'
                else:
                    geo_burst_pol_filename = os.path.join(
                        output_path_child, burst_id, f'{burst_product_id}_{pol}.' + f'{imagery_extension}'
                    )
            ...
            gdal.BuildVRT(geo_burst_vrt_filename, output_burst_imagery_list, options=vrt_options_mosaic)
        ...
        if flag_call_radar_grid:
            ...
            for layer_name, flag_save in radar_grid_layer_dict.items():
                ...
                current_file = os.path.join(
                    output_dir, burst_id, f'{burst_product_id}_{layer_name}.{imagery_extension}'
                )
                if flag_bursts_secondary_files_are_temporary:
