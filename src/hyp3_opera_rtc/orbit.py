    ...
    for orbit_type in orbit_types:
        for provider in providers:
            try:
                ...
                if provider == 'ESA':
                    esa_credentials = utils.get_esa_credentials()
                    with EsaToken(*esa_credentials) as token:
                        orbit_file = utils.download_file(url, directory=directory, token=token)
                else:
                    orbit_file = utils.download_file(url, directory=directory)
                if orbit_file:
                    return Path(orbit_file)
