...
def test_get_xml_with_asf_lineage(iso_xml):
    updated_iso_xml = upload_rtc.get_xml_with_asf_lineage(iso_xml.read())

    assert 'OPERA L2 RTC product generated by ASF using Sentinel-1 input data and SAS version 1.0.2 via HyP3 OPERA-RTC v0.1.4' in updated_iso_xml
    print(updated_iso_xml)


@pytest.fixture
def iso_xml():
    with (Path(__file__).parent / 'data' / 'opera_v1.0.iso.xml').open() as f:
        yield f
