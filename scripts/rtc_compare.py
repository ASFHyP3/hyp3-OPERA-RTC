#!/usr/bin/env python
"""OPERA RTC comparison script created by JPL OPERA RTC team.
...
LIST_EXCLUDE_COMPARISON = [
    ...
    '//metadata/processingInformation/inputs/l1SlcGranules',  # ASF added for known burst2safe changes
]
...
LIST_EXCLUDE_COMPARISON_PRODUCT = [
    ...
    'INPUTS_ORBIT_FILES',
    'PROCESSING_DATETIME',
    'INPUT_ANNOTATION_FILES',  # ASF added for known burst2safe changes
    'INPUT_L1_SLC_GRANULES',  # ASF added for known burst2safe changes
]
...
def print_data_difference(val_1, val_2, indent=4):
    ...
    if not issubclass(val_1.dtype.type, np.number):
        ...
        print(
            f'{str_indent} The first discrepancy has detected from index '
    ...
    print(
        f'{str_indent} Maximum difference detected from index '
        f'{index_2d_max_diff}: '
        f'1st: ({val_1[index_2d_max_diff]}), 2nd: ({val_2[index_2d_max_diff]}) = '
        f'diff: ({difference_val[index_2d_max_diff]})'
    )

    if not (issubclass(val_1.dtype.type, np.floating) or issubclass(val_1.dtype.type, np.complexfloating)):
        return
    ...
    print(
        f'{str_indent} Found {num_pixel_nan_discrepancy} '
        'NaN inconsistencies between input arrays. '
        'First index of the discrepancy: '
        f'[{index_pixel_nan_discrepancy[0][0]}]'
    )
    print(f'{str_indent} val_1[{index_pixel_nan_discrepancy[0][0]}] = {val_1[index_pixel_nan_discrepancy[0][0]]}')
    print(f'{str_indent} val_2[{index_pixel_nan_discrepancy[0][0]}] = {val_2[index_pixel_nan_discrepancy[0][0]]}')
...
def compare_hdf5_elements(
    ...
    if len(shape_val_1) == 0:
        ...
        if issubclass(val_1.dtype.type, np.number):
            ...
            if return_val:
                ...
                print(f'{FAILED_STR} ', str_message_data_location)
                print(
                    '    - numerical scalar. Failed to pass the test. '
    ...
    if len(shape_val_1) == 1:
        ...
        if issubclass(val_1.dtype.type, np.number):
            ...
            if return_val:
                ...
                print(f'{FAILED_STR} ', str_message_data_location)
                print(
                    '    - Numerical 1D array. Failed to pass the test. '
                    f'Relative tolerance = {RTC_S1_PRODUCTS_ERROR_REL_TOLERANCE}, '
                    f'Absolute tolerance = {RTC_S1_PRODUCTS_ERROR_ABS_TOLERANCE}'
                )
    ...
    if len(shape_val_1) >= 2:
        ...
        if return_val:
            ...
            print(f'{FAILED_STR} ', str_message_data_location)
            print(
                f'    {len(shape_val_1)}D raster array. Failed to pass the test. '
                f'Relative tolerance = {RTC_S1_PRODUCTS_ERROR_REL_TOLERANCE}, '
                f'Absolute tolerance = {RTC_S1_PRODUCTS_ERROR_ABS_TOLERANCE}'
            )
            print_data_difference(val_1, val_2)
...
def compare_rtc_hdf5_files(file_1: str, file_2: str, list_elements_to_exclude: list = None):
    ...
    with h5py.File(file_1, 'r') as hdf5_in_1, h5py.File(file_2, 'r') as hdf5_in_2:
        ...
        flag_identical_dataset_structure = len(intersection_set_dataset) == len(set_dataset_1) and len(
            intersection_set_dataset
        ) == len(set_dataset_2)
        ...
        flag_identical_attrs_structure = len(intersection_set_attrs) == len(set_attrs_1) and len(
            intersection_set_attrs
        ) == len(set_attrs_2)
        ...
        final_result = (
            ...
            and flag_identical_attrs_structure
            and flag_same_dataset
            and flag_same_attributes
        )
        ...
        if (not flag_identical_attrs_structure) and flag_identical_dataset_structure:
            print(f'    {FAILED_STR} Attribute structure not identical.')
            print('In the 1st HDF5, not in the 2nd data:')
            print('\r    ' + '\r    '.join(list_attrs_1st_only).replace('\n', ',\tattr: ').replace('\r', '\n'))

            print('\nIn the 2nd HDF5, not in the 1st data:')
            print('\r    ' + '\r    '.join(list_attrs_2nd_only).replace('\n', ',\tattr: ').replace('\r', '\n'))
        ...
        if all(list_flag_identical_dataset):
            print(f'    {PASSED_STR} The datasets of the two HDF files are the same within the tolerance.')
            print(
                f'            Relative tolerance = {RTC_S1_PRODUCTS_ERROR_REL_TOLERANCE}, '
                f'Absolute tolerance = {RTC_S1_PRODUCTS_ERROR_ABS_TOLERANCE}'
            )
        ...
        if all(list_flag_identical_attrs):
            print(f'    {PASSED_STR} The attributes of the two HDF files are the same within the tolerance')
            print(
                f'            Relative tolerance = {RTC_S1_PRODUCTS_ERROR_REL_TOLERANCE}, '
                f'Absolute tolerance = {RTC_S1_PRODUCTS_ERROR_ABS_TOLERANCE}'
            )
...
def compare_rtc_s1_products(file_1, file_2):
    ...
    if not flag_same_nbands:
        print(prefix + f'Input 1 has {nbands_1} bands and input 2 has {nbands_2} bands')
        return False
    ...
    for b in range(1, nbands_1 + 1):
        ...
        if shape_val_1 != shape_val_2:
            # Dataset or attribute shape does not match
            print(f'{FAILED_STR} data shapes do not match. {shape_val_1} vs. {shape_val_2}\n')
            return False

        if image_1.dtype != image_2.dtype:
            print(f'{FAILED_STR} data types do not match. ({image_1.dtype}) vs. ({image_2.dtype})\n')
            return False
        ...
        flag_bands_are_equal_str = _get_prefix_str(flag_bands_are_equal, flag_all_ok)
        print(f'{flag_bands_are_equal_str}     Band {b} - {gdal_band_1.GetDescription()}"')
    ...
    if not flag_same_geotransforms:
        print(
            prefix + f'* input 1 geotransform with content "{geotransform_1}"'
            f' differs from input 2 geotransform with content'
            f' "{geotransform_2}".'
        )

    # compare metadata
    metadata_error_message, flag_same_metadata = _compare_rtc_s1_metadata(metadata_1, metadata_2)

    flag_same_metadata_str = _get_prefix_str(flag_same_metadata, flag_all_ok)
...
def _compare_rtc_s1_metadata(metadata_1, metadata_2):
    ...
    if not flag_same_metadata:
        ...
        if len(set_1_m_2) > 0:
            metadata_error_message += f' Input 1 metadata has extra entries with keys: {", ".join(set_1_m_2)}.'
        set_2_m_1 = set(metadata_2.keys()) - set(metadata_1.keys())
        if len(set_2_m_1) > 0:
            metadata_error_message += f' Input 2 metadata has extra entries with keys: {", ".join(set_2_m_1)}.'
    else:
        for (
            k1,
            v1,
            ...
            if k1 not in metadata_2.keys():
                ...
                metadata_error_message = f'* the metadata key {k1} is present in but it is not present in input 2'
                break
...
def _print_first_value_diff(image_1, image_2, prefix):
    ...
    for i in range(image_1.shape[0]):
        for j in range(image_1.shape[1]):
            if np.isnan(image_1[i, j]) and np.isnan(image_1[i, j]):
                continue
            if abs(
                image_1[i, j] - image_2[i, j]
            ) <= RTC_S1_PRODUCTS_ERROR_ABS_TOLERANCE + RTC_S1_PRODUCTS_ERROR_REL_TOLERANCE * abs(image_2[i, j]):
                continue
            print(
...
def main():
    ...
    for file_1 in file_list_1:
        ...
        if file_1.endswith('.tif'):
            ...
            file_2 = [s for s in file_list_2 if s.endswith(layer_suffix)][0]
        else:
            file_extension = file_1.split('.')[-1]
            file_2 = [s for s in file_list_2 if s.endswith(file_extension)][0]
        ...
        if file_1.endswith('.h5'):
            ...
            results_dict[basename_1] = compare_rtc_hdf5_files(file_1, file_2, LIST_EXCLUDE_COMPARISON)
            continue
